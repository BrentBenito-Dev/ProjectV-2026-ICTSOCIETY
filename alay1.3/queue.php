...
// ONLY SHOW SONGS VALIDATED BY ADMIN
$sql = "SELECT * FROM orders WHERE Status = 'Approved' ORDER BY Timestamp ASC";
        ...
        <div class="space-y-4">
            <?php while($row = $result->fetch_assoc()): ?>
                <div class="relative group flex items-center justify-between p-6 bg-zinc-900 border border-zinc-800 rounded-3xl transition-all hover:border-rose-500/50 hover:-translate-y-1">
                    ...
                    <div class="text-[10px] text-zinc-700 font-mono font-bold">#<?php echo $row['OrderID']; ?></div>
                </div>
            <?php endwhile; ?>
        </div>
    </div>
