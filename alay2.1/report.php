...
// Change 'Finished' to 'COMPLETED'
$total_query = $conn->query("SELECT SUM(Price) as total FROM orders WHERE Status IN ('COMPLETED', 'Voided')");
...
// Change these keys to match the "Service Rate" column in your table
$categories = [
    'Acoustic Live Band' => 89.00, // Updated from 'Acoustic Request'
    'Acoustic Sundo'    => 49.00,
    'Harana Accompaniment'    => 19.00,
    'Karaoke Jam'       => 15.00
];
...
<body class="bg-zinc-100 min-h-screen p-4 md:p-10 font-sans">
    ...
    // This query now counts both Completed and Voided items for this specific category
    $res = $conn->query("SELECT COUNT(*) as qty, SUM(Price) as subtotal FROM orders WHERE Rate_Type = '$name' AND Status IN ('COMPLETED', 'Voided')");
...
?>
    <div class="p-4 bg-zinc-50 border border-zinc-100 rounded-xl">
        ...
        <p class="text-[9px] text-rose-500 font-bold mt-1">Total Qty: <?php echo $data['qty'] ?? 0; ?></p>
    </div>
<?php endforeach; ?>
            </div>
        </div>
        ...
        <div class="mb-12">
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                ...
                <div class="p-4 border-l-4 border-green-400 bg-green-50 rounded-r-xl">
                    ...
                    <p class="text-2xl font-black text-green-700"><?php echo getStatusCount($conn, 'COMPLETED'); ?></p>
                </div>
